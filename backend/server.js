      ...
      try {
        ...
        const [msgResult] = await connection.execute(
          `INSERT INTO chat_messages (conversation_id, sender_id, sender_type, message, is_read, created_at)
           VALUES (?, ?, ?, ?, 0, NOW())`,
          [convId, senderId, senderRole === 'admin' ? 'admin' : 'user', message.trim()]
        );
...
app.post("/api/auth/forgot-password", async (req, res, next) => {
  try {
    ...
    const resetLink = `${process.env.FRONTEND_USER_URL || "https://cryptopulse-4rhe.onrender.com"}/reset-password?token=${resetToken}`;
...
// ✅ NEW: Get user's real assets from database (CryptoPulse)
...
// ✅ NEW: Get user's portfolio assets directly
app.get("/api/user/portfolio-assets-direct", authenticateUser, async (req, res, next) => {
  try {
    const userId = req.user.id;
    ...
    // Get assets from user_assets
    const [assetRows] = await pool.execute(
      `SELECT coin, balance, avg_price FROM user_assets WHERE user_id = ? AND balance > 0`,
      [userId]
    );

    // Get main USDT balance
    const [userRows] = await pool.execute(`SELECT balance FROM users WHERE id = ?`, [userId]);
    const mainBalance = Number(userRows[0]?.balance || 0);
    ...
    // Add other assets
    for (const asset of assetRows) {
      if (asset.coin === "USDT") continue;
      const currentPrice = Number(priceMap.get(`${asset.coin}USDT`) || 0);
      const amount = Number(asset.balance);
      const avgPrice = Number(asset.avg_price || currentPrice);
      const usdtValue = amount * currentPrice;
      const spotPnl = (currentPrice - avgPrice) * amount;
      const invested = avgPrice * amount;
      const spotPnlPercent = invested > 0 ? (spotPnl / invested) * 100 : 0;
...
// Replace old app.listen with server.listen for Socket.io
