...
function FloatingActionItem({ icon: Icon, label, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex min-w-[110px] items-center justify-center gap-2 rounded-2xl border border-white/10 bg-[#0d172b] px-4 py-3 text-sm font-semibold text-white shadow-[0_8px_24px_rgba(0,0,0,0.28)] transition hover:bg-[#13203a]"
    >
...
export default function MobileBottomNav() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-white/10 bg-[#081223]/95 backdrop-blur-xl">
      <div
        ref={menuRef}
      ...
      >
        ...
        <div className="relative flex justify-center">
          ...
          >
            <div
              className={`relative flex h-16 w-16 items-center justify-center rounded-full text-black shadow-[0_0_35px_rgba(163,230,53,0.35)] transition ${
                tradeActive || tradeMenuOpen ? "bg-lime-400" : "bg-lime-300/90"
              }`}
            >
              <ArrowRightLeft size={26} strokeWidth={2.4} />
              <div className="absolute -right-1 -top-1 flex h-5 w-5 items-center justify-center rounded-full bg-[#081223] text-white">
                <ChevronUp
                  size={12}
