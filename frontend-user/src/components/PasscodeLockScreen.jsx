...
export default function PasscodeLockScreen({ onUnlock }) {
    ...
    return Array.from({ length: 4 }).map((_, index) => (
      <div
        ...
        className={`h-3 w-3 rounded-full ${
          index < String(passcode).length ? "bg-lime-400" : "bg-white/10"
        }`}
      />
    ));
  }, [passcode]);
  ...
  if (checkingStatus || !hasPasscode) {
    return (
      <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black px-4">
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-[radial-gradient(circle_at_top_right,rgba(163,230,53,0.08),transparent_18%),linear-gradient(180deg,#000000_0%,#050505_100%)] px-4">
      <div className="w-full max-w-sm rounded-[34px] border border-white/10 bg-[#0b0b0b] p-7 shadow-[0_24px_80px_rgba(0,0,0,0.5)]">
        <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full border border-lime-400/20 bg-lime-400/10 text-lime-300">
          <LockKeyhole size={26} />
        </div>
        ...
        <form onSubmit={handleUnlock} className="mt-5 space-y-4">
          <input
            ...
            className="w-full rounded-2xl border border-white/10 bg-[#141414] px-4 py-4 text-center text-xl tracking-[0.35em] text-white outline-none focus:border-lime-400"
          />
